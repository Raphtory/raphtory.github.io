...
g.add_static_property({"name":"Example Graph"})
g.add_vertex_properties(id="User 1",properties={"date of birth":datetime.strptime("1990-02-03", "%Y-%m-%d")})
g.add_edge_properties(src="User 1", dst="User 2",properties={"data source":"https://link-to-repo.com"})
...
g = Graph()
g.add_edge(timestamp=1,src="Person 1",dst="Person 2",properties={"weight":10},layer="Friends")
g.add_edge(timestamp=2,src="Person 1",dst="Person 2",properties={"weight":13},layer="Friends")
g.add_edge(timestamp=3,src="Person 1",dst="Person 2",properties={"weight":20},layer="Co Workers")
g.add_edge(timestamp=4,src="Person 1",dst="Person 2",properties={"weight":17},layer="Friends")
g.add_edge(timestamp=5,src="Person 1",dst="Person 2",properties={"weight":35},layer="Family")
...
edges_df['timestamp'] = pd.to_datetime(edges_df['timestamp']).astype('datetime64[ms, UTC]')

vertices_df = pd.read_csv("data/network_traffic_vertices.csv")
vertices_df['timestamp'] = pd.to_datetime(vertices_df['timestamp']).astype('datetime64[ms, UTC]')
...
g = Graph.load_from_pandas(edges_df=edges_df,src="source",dst="destination",time="timestamp",
                           props=["data_size_MB"],layer_in_df="transaction_type",
                           const_props=["is_encrypted"],shared_const_props={"datasource":"data/network_traffic_edges.csv"},
                           vertex_df=vertices_df,vertex_col="server_id",vertex_time_col="timestamp",
                           vertex_props=["OS_version","primary_function","uptime_days"],
                           vertex_const_props=["server_name","hardware_type"],vertex_shared_const_props={"datasource":"data/network_traffic_edges.csv"})
...
g=Graph()
g.load_edges_from_pandas(edge_df=edges_df,src_col="source",dst_col="destination",time_col="timestamp",
                           props=["data_size_MB"],layer_in_df="transaction_type",
                           const_props=["is_encrypted"],shared_const_props={"datasource":"data/network_traffic_edges.csv"})

g.load_vertices_from_pandas(vertices_df=vertices_df,vertex_col="server_id",time_col="timestamp",
                           props=["OS_version","primary_function","uptime_days"],
                           const_props=["server_name","hardware_type"],shared_const_props={"datasource":"data/network_traffic_edges.csv"})
...
g.load_edge_props_from_pandas(edge_df=edges_df,src_col="source",dst_col="destination",layer_in_df="transaction_type",
                              const_props=["is_encrypted"],shared_const_props={"datasource":"data/network_traffic_edges.csv"})

g.load_vertex_props_from_pandas(vertices_df=vertices_df,vertex_col="server_id",
                                const_props=["server_name","hardware_type"],shared_const_props={"datasource":"data/network_traffic_edges.csv"})
